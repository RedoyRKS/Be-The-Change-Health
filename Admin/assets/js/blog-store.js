...
(function (root, factory) {
  ...
  function getAdminFrontendPrefix() {
    ...
    if (pagesIdx !== -1) {
      ...
      // Subfolder under /pages/ (e.g. pages/blog/blog.html) -> depth 2 inside Admin
      if (rest.indexOf("/") !== -1) {
        return "../../";
      }
  ...
  var DEFAULT_PAGE_CONTENT = {
    ...
    seo: {
      ...
      ogImage: "assets/uploads/2024/08/What-is-Microcurrent-1024x683-1.jpg"
    }
  };
      ...
      parsed.forEach(function (p) {
        for (var i = 0; i < DEFAULT_POSTS.length; i++) {
          if (DEFAULT_POSTS[i].id === p.id) {
            var hasBrokenPrefix = typeof p.image === "string" && (
  ...
  function getRawPageContent() {
    try {
      ...
      var featHasBroken = typeof parsed.featured?.image === "string" && (
        parsed.featured.image.indexOf("Frontend/") !== -1 ||
        parsed.featured.image.indexOf("data:image/svg") === 0 ||
        parsed.featured.image.indexOf("../assets/") === 0 ||
        parsed.featured.image.indexOf("../../assets/") === 0
      );
      if (parsed.featured && (!parsed.featured.image || typeof parsed.featured.image !== "string" || parsed.featured.image.trim() === "" || featHasBroken)) {
        parsed.featured.image = DEFAULT_PAGE_CONTENT.featured.image;
        pageChanged = true;
      }
      var ogHasBroken = typeof parsed.seo?.ogImage === "string" && (
        parsed.seo.ogImage.indexOf("Frontend/") !== -1 ||
        parsed.seo.ogImage.indexOf("data:image/svg") === 0 ||
        parsed.seo.ogImage.indexOf("../assets/") === 0 ||
        parsed.seo.ogImage.indexOf("../../assets/") === 0
      );
      if (parsed.seo && (!parsed.seo.ogImage || typeof parsed.seo.ogImage !== "string" || parsed.seo.ogImage.trim() === "" || ogHasBroken)) {
