...
(function (root, factory) {
  ...
  function getAdminFrontendPrefix() {
    ...
    if (pagesIdx !== -1) {
      ...
      // Subfolder under /pages/ (e.g. pages/blog/blog.html) -> depth 2 inside Admin -> 3 levels up to root
      if (rest.indexOf("/") !== -1) {
        return "../../../Frontend/";
      }
  ...
  var DEFAULT_PAGE_CONTENT = {
    hero: {
      ...
      subtitle: "Education and insights on integrative therapies, wellness, and whole-person care.",
      breadcrumb: "Blog"
    },
    intro: {
      searchPlaceholder: "Search articles…",
      categories: ["All", "Therapies", "Wellness", "Detox", "Blog"],
      emptyMessage: "No articles match your search. Try another keyword or category."
    },
    featured: {
      enabled: true,
      badgeText: "Featured Article",
      postId: "what-is-frequency-specific-microcurrent-therapy",
      category: "Therapies",
      ...
      link: "what-is-frequency-specific-microcurrent-therapy.html",
      image: "../../../Frontend/assets/uploads/2024/08/What-is-Microcurrent-1024x683-1.jpg"
    },
    articlesHeader: {
      title: "All Articles",
      subtitle: "Explore our wellness library.",
      readMoreText: "Read More"
    },
    cta: {
      title: "Ready to experience these therapies?",
      description: "Visit our Columbia, MD clinic or request an appointment online.",
    ...
    seo: {
      title: "Blog - Be The Change Health and Wellness Center",
      description: "Be The Change Blog — wellness articles on microcurrent, hyperbaric oxygen, infrared sauna, and ionic foot detox from our Columbia, MD clinic.",
      keywords: "integrative medicine blog, health and wellness articles, microcurrent therapy, hyperbaric oxygen, infrared sauna, foot detox, Columbia MD",
      canonicalUrl: "Frontend/blog/index.html",
      ogTitle: "Be The Change Health & Wellness Blog",
      ogDescription: "Education and insights on integrative therapies, wellness, and whole-person care in Columbia, MD.",
      ogImage: "../../../Frontend/assets/uploads/2024/08/What-is-Microcurrent-1024x683-1.jpg"
    }
  };
  ...
  var DEFAULT_POSTS = [
    ...
    },
    {
      ...
      image: "../../../Frontend/assets/uploads/2024/08/HYPERBARIC-OXYGEN-THERAPY-1024x683-1.jpg",
      category: "Therapies",
    ...
    },
    {
      ...
      image: "../../../Frontend/assets/uploads/2024/08/Redlight-Therapy-1024x683-1.jpg",
      category: "Wellness",
    ...
    },
    {
      ...
      image: "../../../Frontend/assets/uploads/2024/08/foot-detox-1024x683-1.jpg",
      category: "Detox",
  ...
  function getRawPosts() {
    try {
      ...
      parsed.forEach(function (p) {
        for (var i = 0; i < DEFAULT_POSTS.length; i++) {
          if (DEFAULT_POSTS[i].id === p.id) {
  ...
  function getRawPageContent() {
    try {
      if (typeof localStorage === "undefined") {
        if (!_memoryPageStore) _memoryPageStore = clone(DEFAULT_PAGE_CONTENT);
        return clone(_memoryPageStore);
      }
      var stored = localStorage.getItem(STORAGE_KEY_PAGE);
      if (!stored) {
        localStorage.setItem(STORAGE_KEY_PAGE, JSON.stringify(DEFAULT_PAGE_CONTENT));
        return clone(DEFAULT_PAGE_CONTENT);
      }
      var parsed = JSON.parse(stored);
      ...
      if (parsed.featured && (!parsed.featured.image ||
          typeof parsed.featured.image !== "string" ||
          parsed.featured.image.indexOf("data:image/svg") === 0 ||
          parsed.featured.image.indexOf("../../Frontend/") === 0 ||
          parsed.featured.image.indexOf("../assets/") === 0 ||
          parsed.featured.image.indexOf("assets/") === 0)) {
        parsed.featured.image = DEFAULT_PAGE_CONTENT.featured.image;
        pageChanged = true;
      }
      if (parsed.seo && (!parsed.seo.ogImage ||
          typeof parsed.seo.ogImage !== "string" ||
          parsed.seo.ogImage.indexOf("data:image/svg") === 0 ||
          parsed.seo.ogImage.indexOf("../../Frontend/") === 0 ||
          parsed.seo.ogImage.indexOf("../assets/") === 0 ||
          parsed.seo.ogImage.indexOf("assets/") === 0)) {
        parsed.seo.ogImage = DEFAULT_PAGE_CONTENT.seo.ogImage;
        pageChanged = true;
      }
      if (pageChanged) {
        localStorage.setItem(STORAGE_KEY_PAGE, JSON.stringify(parsed));
      }
      var merged = Object.assign({}, clone(DEFAULT_PAGE_CONTENT), parsed);
      return merged;
    } catch (e) {
      console.warn("BTCBlogStore: localStorage unavailable for page content", e);
      if (!_memoryPageStore) _memoryPageStore = clone(DEFAULT_PAGE_CONTENT);
      return clone(_memoryPageStore);
    }
  }

  function persistPageContent(content) {
    _memoryPageStore = clone(content);
    try {
      if (typeof localStorage !== "undefined") {
        localStorage.setItem(STORAGE_KEY_PAGE, JSON.stringify(content));
      }
      if (typeof window !== "undefined" && typeof window.dispatchEvent === "function") {
        window.dispatchEvent(new CustomEvent("btc-blog-page-updated", { detail: { pageContent: content } }));
      }
      return true;
    } catch (e) {
      console.error("BTCBlogStore: persistPageContent failed", e);
      return false;
    }
  }
  ...
  var BTCBlogStore = {
    ...
    ARTICLE_DEFAULT_IMAGES: ARTICLE_DEFAULT_IMAGES,
    getAdminFrontendPrefix: getAdminFrontendPrefix,

    resolveImage: function (imgUrl, callerContext, postId) {
      if ((!imgUrl || typeof imgUrl !== "string" || !imgUrl.trim() || imgUrl.indexOf("data:image/svg") === 0) && postId && ARTICLE_DEFAULT_IMAGES[postId]) {
        imgUrl = ARTICLE_DEFAULT_IMAGES[postId];
      }
      ...
      if (trimmed.indexOf("data:") === 0) {
        if (postId && ARTICLE_DEFAULT_IMAGES[postId]) {
          trimmed = ARTICLE_DEFAULT_IMAGES[postId];
        } else {
          return trimmed;
        }
      }
      ...
      if (postId && ARTICLE_DEFAULT_IMAGES[postId]) {
        var def = ARTICLE_DEFAULT_IMAGES[postId];
        var prefix2 = callerContext === "frontend" ? "../" : getAdminFrontendPrefix();
        return prefix2 + def;
      }
      return trimmed;
    },
    ...
    getClinicMedia: function () {
      var prefix = getAdminFrontendPrefix();
      return [
        { url: prefix + "assets/uploads/2024/08/What-is-Microcurrent-1024x683-1.jpg", name: "What-is-Microcurrent.jpg", title: "Microcurrent Therapy", cat: "Therapies" },
        { url: prefix + "assets/uploads/2024/08/HYPERBARIC-OXYGEN-THERAPY-1024x683-1.jpg", name: "HYPERBARIC-OXYGEN-THERAPY.jpg", title: "Hyperbaric Oxygen Therapy", cat: "Therapies" },
        { url: prefix + "assets/uploads/2024/08/Redlight-Therapy-1024x683-1.jpg", name: "Redlight-Therapy.jpg", title: "Infrared Sauna & Red Light", cat: "Wellness" },
        { url: prefix + "assets/uploads/2024/08/foot-detox-1024x683-1.jpg", name: "foot-detox.jpg", title: "Ion Foot Detox", cat: "Detox" },
        { url: prefix + "assets/uploads/2024/08/OX.jpg", name: "OX.jpg", title: "Hyperbaric Chamber Room", cat: "Modalities" },
        { url: prefix + "assets/uploads/2024/08/SAUNA.jpg", name: "SAUNA.jpg", title: "Infrared Sauna Unit", cat: "Modalities" },
        { url: prefix + "assets/uploads/2024/08/DETOX.jpg", name: "DETOX.jpg", title: "Ion Foot Detox Station", cat: "Modalities" },
        { url: prefix + "assets/uploads/2024/08/IV.jpg", name: "IV.jpg", title: "IV Infusion Lounge", cat: "Modalities" },
        { url: prefix + "assets/uploads/2024/08/sa.jpg", name: "sa.jpg", title: "Dr. Sultana Afrooz, D.O.", cat: "Physicians" },
        { url: prefix + "assets/uploads/2025/02/jn-5.jpg", name: "jn-5.jpg", title: "Dr. Jessica Needle, N.D.", cat: "Physicians" },
        { url: prefix + "assets/uploads/2025/02/jessica.jpg", name: "jessica.jpg", title: "Dr. Jessica Profile", cat: "Physicians" },
        { url: prefix + "assets/uploads/2024/08/approach.jpg", name: "approach.jpg", title: "Whole-Person Approach", cat: "About" },
        { url: prefix + "assets/uploads/2024/10/hh.jpg", name: "hh.jpg", title: "Integrative Health Consultation", cat: "About" },
        { url: prefix + "assets/uploads/2024/10/therapeutic-order.png", name: "therapeutic-order.png", title: "Therapeutic Order Chart", cat: "Clinical" },
        { url: prefix + "assets/uploads/2024/10/Vision.jpg", name: "Vision.jpg", title: "Clinic Vision", cat: "Pillars" },
        { url: prefix + "assets/uploads/2024/10/mission2.jpg", name: "mission2.jpg", title: "Mission & Purpose", cat: "Pillars" },
        { url: prefix + "assets/uploads/2024/10/core.jpg", name: "core.jpg", title: "Core Values", cat: "Pillars" },
        { url: prefix + "assets/uploads/2025/06/home.jpg", name: "home.jpg", title: "Clinic Entrance / Reception", cat: "Facilities" },
        { url: prefix + "assets/uploads/2024/08/LOGO.jpg", name: "LOGO.jpg", title: "Be The Change Logo", cat: "Branding" }
      ];
    },
    ...
    /* =========================================================================
       BLOG PAGE CMS CONTENT MANAGEMENT
       ========================================================================= */

    getPageContent: function () {
      return getRawPageContent();
    },

    savePageContent: function (content) {
      var merged = Object.assign({}, clone(DEFAULT_PAGE_CONTENT), content);
      persistPageContent(merged);
      return clone(merged);
    },

    saveHero: function (heroData) {
      var content = getRawPageContent();
      content.hero = Object.assign({}, content.hero, heroData);
      persistPageContent(content);
      return clone(content.hero);
    },

    saveIntro: function (introData) {
      var content = getRawPageContent();
      content.intro = Object.assign({}, content.intro, introData);
      persistPageContent(content);
      return clone(content.intro);
    },

    saveFeaturedSection: function (featuredData) {
      var content = getRawPageContent();
      content.featured = Object.assign({}, content.featured, featuredData);
      persistPageContent(content);
      return clone(content.featured);
    },

    saveArticlesHeader: function (headerData) {
      var content = getRawPageContent();
      content.articlesHeader = Object.assign({}, content.articlesHeader, headerData);
      persistPageContent(content);
      return clone(content.articlesHeader);
    },

    saveCta: function (ctaData) {
      var content = getRawPageContent();
      content.cta = Object.assign({}, content.cta, ctaData);
      persistPageContent(content);
      return clone(content.cta);
    },

    saveSeo: function (seoData) {
      var content = getRawPageContent();
      content.seo = Object.assign({}, content.seo, seoData);
      persistPageContent(content);
      return clone(content.seo);
    },

    resetPageContentToDefaults: function () {
      var def = clone(DEFAULT_PAGE_CONTENT);
      persistPageContent(def);
      return def;
    }
  };
